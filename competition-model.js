...
export function buildEventArchive(state,competition={},options={}){
 ...
 return {name:event.name||"MFMA Event",seasonId:options.seasonId,pointsEvent:event.pointsEvent!==false,date:options.date||new Date().toISOString().slice(0,10),endedAt:options.endedAt||Date.now(),classification,sessionResults,participationAwards,passengerParticipation,sessionCount:sessionResults.length||Math.max(0,(event.sessionNumber||1)-1),circuit:event.circuit||null,vehicleReplacements:event.vehicleReplacements||null,scoringPolicy:{sessionPoints:[...sessionPoints],eventPoints:[...points],passengerSeasonCap:PASSENGER_SEASON_CAP}};
}
...
export function rebuildStandings(archives={}){
 ...
 const validArchives=Object.values(archives||{}).filter(archive=>archive?.pointsEvent!==false&&archive?.status!=="void").sort((a,b)=>(a.endedAt||0)-(b.endedAt||0)),passengerTotals={};
