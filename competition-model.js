...
export function buildEventArchive(state,competition={},options={}){
 ...
 if(event.eventFormat==="qualifying"){const validLaps=Object.values(event.qualifying?.laps||{}).filter(lap=>lap?.status!=="invalid"&&Number(lap?.timeMs)>0),winnerId=event.qualifying?.winner?.driverId||validLaps.sort((a,b)=>Number(a.timeMs)-Number(b.timeMs))[0]?.driverId;for(const lap of validLaps){const driver=competition?.drivers?.[lap.driverId];if(!lap.driverId||participationByDriver[lap.driverId]||driver?.status!=="approved")continue;participationByDriver[lap.driverId]={driverId:lap.driverId,driverName:driver.name||lap.driverName,mraNumber:driver.mraNumber||lap.mraNumber,role:"qualifying",points:lap.driverId===winnerId?2:1,fastestLap:lap.driverId===winnerId}}
 }
