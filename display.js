...
function renderVehicleOptions(){const ids=Object.keys(LEGACY_VEHICLES),prior=selectedVehicle;if(!ids.includes(selectedVehicle))selectedVehicle="ranger";vehicleSelect.innerHTML=ids.map(id=>`<option value="${escapeHtml(id)}">${escapeHtml(LEGACY_VEHICLES[id].name)}</option>`).join("");vehicleSelect.value=ids.includes(prior)?prior:selectedVehicle}
...
function storeDriverProfile(profile){driverProfile=profile;selectedVehicle=profile.vehicleId||selectedVehicle;renderVehicleOptions();vehicleSelect.value=selectedVehicle;localStorage.setItem(PROFILE_KEY,JSON.stringify({...profile,vehicleId:selectedVehicle}));localStorage.setItem("mfma-driver-vehicle",selectedVehicle);renderCrewChoices()}
async function submitDriverProfile(){if(!state?.event||!driverProfile)return;await ensureDriverAuth();const report={...cleanOccupantReport(driverProfile.driverName,driverProfile.passengerName,serverTimestamp()),driverId:driverProfile.driverId,mraNumber:driverProfile.mraNumber,teamId:driverProfile.teamId,teamName:driverProfile.teamName,passengerId:driverProfile.passengerId||null,passengerMraNumber:driverProfile.passengerId?competition.drivers?.[driverProfile.passengerId]?.mraNumber||null:null},updates={[`vehicleReports/${driverProfile.vehicleId}`]:report,[`driverParticipants/${driverProfile.driverId}`]:{driverId:driverProfile.driverId,driverName:driverProfile.driverName,mraNumber:driverProfile.mraNumber,teamId:driverProfile.teamId,teamName:driverProfile.teamName,vehicleId:driverProfile.vehicleId,lastParticipatedAt:serverTimestamp()}};if(driverProfile.passengerId){const passenger=competition.drivers?.[driverProfile.passengerId];updates[`passengerParticipants/${driverProfile.passengerId}`]={driverId:driverProfile.passengerId,driverName:passenger.name,mraNumber:passenger.mraNumber,recordedByTeamId:driverProfile.teamId,participatedAt:serverTimestamp()}}await update(ref(db,"mfma/state/event"),updates)}
