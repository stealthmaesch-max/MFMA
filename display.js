...
import { LEGACY_VEHICLES, normalizeMraNumber, normalizeName, currentSeasonId, rebuildStandings } from "./competition-model.js?v=79";
...
function renderTeamChoices(){const teams=Object.entries(competition.teams||{}).filter(([,team])=>team.status==="approved").sort((a,b)=>a[1].name.localeCompare(b[1].name));const prior=$("signin-team").value;$("signin-team").innerHTML=teams.map(([id,team])=>`<option value="${escapeHtml(id)}">${escapeHtml(team.name)}</option>`).join("");if(teams.some(([id])=>id===prior))$("signin-team").value=prior}
...
onValue(ref(db,"mfma/competition"),snapshot=>{competition=snapshot.val()||{};renderVehicleOptions();renderTeamChoices();if(driverProfile?.driverId){const driver=competition.drivers?.[driverProfile.driverId];if(driver)storeDriverProfile({...driverProfile,driverName:driver.name,mraNumber:driver.mraNumber,teamId:driver.teamId,teamName:driver.teamName||competition.teams?.[driver.teamId]?.name||"Team"})}activateApprovedBinding();loadVehicleReports()});
