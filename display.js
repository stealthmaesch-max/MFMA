...
import { LEGACY_VEHICLES, applyStandingsAdjustments, isScoringDriverEligible, normalizeMraNumber, normalizeName, currentSeasonId, rebuildStandings } from "./competition-model.js?v=83";
...
function openStats(){
 ...
 $("portal-sheet-title").textContent=driverProfile.driverName;$("portal-sheet-content").innerHTML=`<h3>${seasonId} Season</h3><div class="portal-stat-grid"><article><strong>${row.points||0}</strong><span>Points</span></article><article><strong>${row.participationPoints||0}</strong><span>Passenger Points</span></article><article><strong>${row.wins||0}</strong><span>Wins</span></article><article><strong>${row.events||0}</strong><span>Events</span></article></div><h3>Career</h3><div class="portal-stat-grid"><article><strong>${career.points||0}</strong><span>Points</span></article><article><strong>${career.participationPoints||0}</strong><span>Passenger Points</span></article><article><strong>${career.wins||0}</strong><span>Wins</span></article><article><strong>${career.events||0}</strong><span>Events</span></article></div><h3>Prior Races</h3><div class="portal-history">${archives.map(event=>{const result=event.classification?.find(entry=>entry.driverId===driverProfile.driverId),passenger=event.passengerParticipation?.some(entry=>entry.driverId===driverProfile.driverId),noPoints=event.status==="void"||event.pointsEvent===false;return `<article><span><strong>${escapeHtml(event.name)}</strong><small>${escapeHtml(event.date||"")} • ${escapeHtml(event.seasonId||"")} • ${event.status==="void"?"VOID":event.pointsEvent===false?"Non-points":"Points event"}</small></span><b>${noPoints?"No points":result?`P${result.position} • ${result.points} pts`:passenger?"Passenger • 1 pt":"—"}</b></article>`}).join("")||"<p>No archived races yet.</p>"}</div>`;$("portal-sheet").classList.remove("hidden")
}
