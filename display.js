...
import { LEGACY_VEHICLES, normalizeMraNumber, normalizeName, currentSeasonId, rebuildStandings } from "./competition-model.js?v=80";
...
async function submitDriverProfile(){if(!state?.event||!driverProfile)return;await ensureDriverAuth();const report={...cleanOccupantReport(driverProfile.driverName,driverProfile.passengerName,serverTimestamp()),driverId:driverProfile.driverId,mraNumber:driverProfile.mraNumber,teamId:driverProfile.teamId,teamName:driverProfile.teamName};if(driverProfile.passengerId)Object.assign(report,{passengerId:driverProfile.passengerId,passengerMraNumber:competition.drivers?.[driverProfile.passengerId]?.mraNumber||""});await update(ref(db,`mfma/state/event/vehicleReports/${driverProfile.vehicleId}`),report);if(driverProfile.passengerId){const passenger=competition.drivers?.[driverProfile.passengerId];await update(ref(db,`mfma/state/event/passengerParticipants/${driverProfile.passengerId}`),{driverId:driverProfile.passengerId,driverName:passenger.name,mraNumber:passenger.mraNumber,recordedByTeamId:driverProfile.teamId,participatedAt:serverTimestamp()})}}
