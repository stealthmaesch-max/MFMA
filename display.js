...
function showLive(){const sig=signals[state.activeFlag]||signals.clear,s=state.session;statusView.classList.add("hidden");liveView.classList.remove("hidden");display.className=`display ${sig.className}${sig.flash?" flash":""}`;label.textContent=sig.label;instruction.textContent=sig.instruction;theme.content=sig.theme;sessionLine.textContent=`SESSION ${s.number} • ${s.phase.toUpperCase()} • PURSUIT: ${s.teamNames[s.pursuitTeam]} • EVADING: ${s.teamNames[s.evadingTeam]}`;timer.textContent=fmt(s.remainingMs)}
