...
function render(){
 ...
 const s=state.session;$("ops-phase").textContent=s?`${s.phase.toUpperCase()} • ${(state.activeFlag||"clear").toUpperCase()}`:"STANDBY";$("ops-timer").textContent=s?fmt(s.remainingMs):"--:--";$("ops-session").textContent=s?`Session ${s.number}`:"";$("ops-roles").textContent=s?`PURSUIT: ${s.teamNames?.[s.pursuitTeam]||"—"} • EVADING: ${s.teamNames?.[s.evadingTeam]||"—"}`:"";
...
onValue(stateRef,s=>{state=s.val()||{};render()},e=>console.error(e));setInterval(()=>{if(state?.session&&state.systemState==="session-live"){const factor=state.session.flag==="yellow"?0.5:1;$("ops-timer").textContent=fmt(Math.max(0,state.session.remainingMs-(Date.now()-(state.session.lastTickAt||Date.now()))*factor))}},250);
