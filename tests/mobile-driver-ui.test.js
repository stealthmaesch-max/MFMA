...
test("Grip deterioration weather treatments fit iPhone 15 portrait",async()=>{
 const browser=await chromium.launch({headless:true}),page=await signedDriverPage(browser,{width:393,height:852});await page.goto(`${baseUrl}/display.html`,{waitUntil:"domcontentloaded"});
 for(const condition of ["rain","debris"]){const result=await page.evaluate(condition=>{document.querySelector("#status-view").classList.add("hidden");document.querySelector("#live-view").classList.remove("hidden");const display=document.querySelector("#display");display.className="display flag-grip flash";display.dataset.gripCondition=condition;document.querySelector("#label").textContent="GRIP DETERIORATION";document.querySelector("#instruction").textContent="GRIP • GRIP • GRIP";document.querySelector("#display-session").textContent="MFMA SPRINT • FLAG OPERATIONS";document.querySelector("#display-timer").textContent="NO FORMAL SESSION";const label=document.querySelector("#label").getBoundingClientRect(),surface=getComputedStyle(display,"::after");return {width:document.documentElement.scrollWidth,label:{left:label.left,right:label.right},condition:display.dataset.gripCondition,animation:surface.animationName,background:surface.backgroundImage}},condition);assert.equal(result.width,393);assert(result.label.left>=0&&result.label.right<=393,`${condition} label stays contained`);assert.equal(result.condition,condition);assert.match(result.animation,/gripSurface/);assert.notEqual(result.background,"none");}
 await browser.close();
});
