...
test("Test Mode controls and display fit iPhone 15 portrait",async()=>{
 ...
 const controlMetrics=await control.evaluate(()=>{document.body.dataset.mode="test-mode";document.querySelector("#secured-control").classList.remove("hidden");document.querySelector("#event-area").classList.remove("hidden");document.querySelector("#test-mode-panel").classList.remove("hidden");const quick=document.querySelector("#quick-flag-panel");quick.classList.remove("hidden");quick.classList.add("track-only");const track=document.querySelector("#track-control");track.classList.remove("hidden");track.open=true;const panel=document.querySelector("#test-mode-panel").getBoundingClientRect(),trackBox=track.getBoundingClientRect(),visibleButtons=[...document.querySelectorAll("[data-test-flag],#track-control button")].filter(button=>getComputedStyle(button).display!=="none"&&button.getBoundingClientRect().height>0);return {width:document.documentElement.scrollWidth,panel:{left:panel.left,right:panel.right},track:{left:trackBox.left,right:trackBox.right,sectors:track.querySelectorAll("[data-track-sector]").length},buttons:visibleButtons.map(button=>button.getBoundingClientRect().height)}});assert.equal(controlMetrics.width,393);assert(controlMetrics.panel.left>=0&&controlMetrics.panel.right<=393);assert(controlMetrics.track.left>=0&&controlMetrics.track.right<=393);assert.equal(controlMetrics.track.sectors,6);assert(controlMetrics.buttons.length>=12,"all visible sector and local-signal controls are measured");assert(controlMetrics.buttons.every(height=>height>=44));await control.close();
...
test("local Grip cycles above Green without replacing the primary flag",async()=>{
 ...
 for(const condition of ["rain","debris"]){const result=await page.evaluate(condition=>{document.querySelector("#status-view").classList.add("hidden");document.querySelector("#live-view").classList.remove("hidden");const display=document.querySelector("#display");display.className="display flag-green grip-advisory has-track-signals";display.dataset.gripCondition=condition;const label=document.querySelector("#label");label.textContent="GO";document.querySelector("#instruction").textContent="SESSION ACTIVE";let panel=document.querySelector("#track-advisory");if(!panel){panel=document.createElement("aside");panel.id="track-advisory";display.append(panel)}panel.className="track-advisory has-grip";panel.innerHTML='<svg class="driver-track-map" viewBox="0 0 320 180"><path class="grip-rain" data-track-sector="sector-2" d="M58 43 C88 15 142 17 170 37"/></svg><div><span>LOCAL TRACK SIGNAL</span><strong>GRIP DETERIORATION</strong></div>';const box=panel.querySelector("strong").getBoundingClientRect(),surface=getComputedStyle(panel,"::before"),panelStyle=getComputedStyle(panel);return {width:document.documentElement.scrollWidth,primary:label.textContent,advisory:{left:box.left,right:box.right,whiteSpace:getComputedStyle(panel.querySelector("strong")).whiteSpace},condition:display.dataset.gripCondition,panelAnimation:panelStyle.animationName,weatherAnimation:surface.animationName,background:surface.backgroundImage}},condition);assert.equal(result.width,375);assert.equal(result.primary,"GO","Green remains the primary flag");assert(result.advisory.left>=0&&result.advisory.right<=375,`${condition} advisory stays contained`);assert.equal(result.condition,condition);assert.match(result.panelAnimation,/gripCycle/);assert.match(result.weatherAnimation,/gripWeatherCycle/);assert.match(result.weatherAnimation,condition==="rain"?/gripRain/:/gripDebris/);assert.notEqual(result.background,"none");}
 await browser.close();
});

test("six-sector MRA track controls fit iPhone 15 portrait",async()=>{
 const browser=await chromium.launch({headless:true}),page=await browser.newPage({viewport:{width:393,height:852}});await page.goto(`${baseUrl}/control.html`,{waitUntil:"domcontentloaded"});await page.waitForSelector("#track-control",{state:"attached"});
 const result=await page.evaluate(()=>{document.querySelector("#auth-gate")?.classList.add("hidden");document.querySelector("#secured-control")?.classList.remove("hidden");document.querySelector("#event-area")?.classList.remove("hidden");const quick=document.querySelector("#quick-flag-panel");quick.classList.remove("hidden");const control=document.querySelector("#track-control");control.classList.remove("hidden");control.open=true;const box=control.getBoundingClientRect(),map=control.querySelector("svg").getBoundingClientRect(),buttons=[...control.querySelectorAll("button")].map(button=>button.getBoundingClientRect());return {width:document.documentElement.scrollWidth,sectors:control.querySelectorAll("[data-track-sector]").length,box:{left:box.left,right:box.right},map:{left:map.left,right:map.right},buttons:buttons.map(item=>({left:item.left,right:item.right,height:item.height}))}});
 assert.equal(result.width,393);assert.equal(result.sectors,6);assert(result.box.left>=0&&result.box.right<=393);assert(result.map.left>=result.box.left&&result.map.right<=result.box.right);assert(result.buttons.every(button=>button.left>=result.box.left&&button.right<=result.box.right&&button.height>=44));await browser.close();
});

test("sector buttons and map routes change the active selection",async()=>{
 const browser=await chromium.launch({headless:true}),page=await browser.newPage({viewport:{width:393,height:852}}),errors=[];page.on("pageerror",error=>errors.push(error.message));await page.goto(`${baseUrl}/control.html`,{waitUntil:"domcontentloaded"});await page.waitForSelector("#track-control",{state:"attached"});
 await page.evaluate(()=>{document.body.dataset.mode="test-mode";document.querySelector("#secured-control").classList.remove("hidden");document.querySelector("#event-area").classList.remove("hidden");document.querySelector("#quick-flag-panel").classList.remove("hidden");const control=document.querySelector("#track-control");control.classList.remove("hidden");control.open=true});
 await page.locator('[data-sector-select="sector-2"]').click();
 let selected=await page.locator('[data-sector-select][aria-pressed="true"]').evaluateAll(buttons=>buttons.map(button=>button.dataset.sectorSelect));assert.deepEqual(selected,["sector-1","sector-2"]);assert.equal(await page.locator('[data-track-sector="sector-2"]').getAttribute("aria-pressed"),"true");
 await page.locator('[data-track-sector="sector-3"]').dispatchEvent("click");selected=await page.locator('[data-sector-select][aria-pressed="true"]').evaluateAll(buttons=>buttons.map(button=>button.dataset.sectorSelect));assert.deepEqual(selected,["sector-1","sector-2","sector-3"]);assert.equal(await page.locator('[data-track-sector="sector-3"]').getAttribute("aria-pressed"),"true");assert.deepEqual(errors,[]);await browser.close();
});
