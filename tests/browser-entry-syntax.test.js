...
test("MRA dual-role controls reuse the linked Driver profile without signing out",()=>{const fs=require("node:fs"),html=fs.readFileSync("control.html","utf8"),source=fs.readFileSync("control.js","utf8");assert.match(html,/id="driver-manager-dialog"/);assert.match(html,/id="driver-manager-open"/);assert.match(source,/mfma\/driverAccess\/\$\{user\.uid\}/);assert.match(source,/function saveManagerDriverProfile/);assert.match(source,/localStorage\.setItem\("mfma-driver-profile"/);assert.doesNotMatch(source,/saveManagerDriverProfile[\s\S]{0,1800}signOut\(/)});

test("Race Control keeps core safety signals visible and secondary signals progressive",()=>{const html=require("node:fs").readFileSync("control.html","utf8");assert.match(html,/id="more-signals"/);assert.match(html,/id="secondary-signals" class="secondary-signals hidden"/);for(const signal of ["yellow","red","safety-car"])assert.match(html,new RegExp(`data-quick-flag="${signal}"`));assert.match(html,/class="event-actions-menu"/);assert.match(html,/class="sprint-advanced-controls"/)});
