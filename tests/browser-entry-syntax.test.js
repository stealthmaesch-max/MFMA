...
test("Race Control buttons avoid unsupported native text prompts and rerender after registry load",()=>{const fs=require("node:fs"),control=fs.readFileSync("control.js","utf8"),html=fs.readFileSync("control.html","utf8");assert.doesNotMatch(control,/\bprompt\s*\(/);assert.match(html,/id="text-entry-dialog"/);assert.match(control,/function requestText/);assert.match(control,/renderChampionshipEventOptions\(\);if\(state\)render\(\)/)});
