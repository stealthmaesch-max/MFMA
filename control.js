...
import { DEFAULT_POINTS, approvedVehicles, buildEventArchive, rebuildStandings, currentSeasonId, normalizeMraNumber, normalizeName, slugify, sortedStandings, teamBranding } from "./competition-model.js?v=80";
...
function currentTeams(){
 if(sessionType==="vehicle-vehicle"){
  ...
  return {a,b,names:{a:vehicleTeamName(a),b:vehicleTeamName(b)}}
 }
...
async function approveRegistration(uid){if(!requireAuthenticatedWrite())return;const item=requests.registrations?.[uid],mraNumber=normalizeMraNumber(document.querySelector(`[data-registration-number="${CSS.escape(uid)}"]`)?.value);if(!item||!mraNumber){alert("Assign an MRA number before approval.");return}const driverId=`mra-${slugify(mraNumber)}`,teamId=slugify(item.teamName)||"independent",teamName=normalizeName(item.teamName);if(competition.drivers?.[driverId]&&competition.drivers[driverId].name!==normalizeName(item.name)){alert("That MRA number is already assigned to another driver.");return}await update(ref(db,"mfma"),{[`competition/drivers/${driverId}`]:{name:normalizeName(item.name),mraNumber,teamId,teamName,status:"approved",approvedAt:serverTimestamp()},[`competition/teams/${teamId}`]:{name:teamName,status:"approved"},[`driverAccess/${uid}`]:{teamId,teamName,status:"approved",approvedAt:serverTimestamp()},[`requests/registrations/${uid}/status`]:"approved",[`requests/registrations/${uid}/approvedAt`]:serverTimestamp()})}
...
function renderStandings(){const season=$("standings-season").value||currentSeasonId(),type=$("standings-type").value,entries=season==="career"?Object.values(competition.archives||{}).flatMap(value=>Object.entries(value||{})):Object.entries(competition.archives?.[season]||{}),source=rebuildStandings(Object.fromEntries(entries)),rows=sortedStandings(source?.[type]);$("standings-list").innerHTML=`<div class="standings-table"><div class="standings-head"><span>Pos</span><span>${type[0].toUpperCase()+type.slice(1)}</span><span>Wins</span><span>Points</span></div>${rows.map((row,index)=>{const teamId=type==="teams"?row.id:type==="drivers"?competition.drivers?.[row.id]?.teamId:competition.vehicles?.[row.id]?.teamId,brand=teamBranding(teamId);return `<article style="--team-accent:${escapeHtml(brand.accent)}"><b>${index+1}</b><span class="standing-name">${brand.emblem?`<img src="${escapeHtml(brand.emblem)}" alt="">`:""}<span>${escapeHtml(row.name)}</span></span><span>${row.wins||0}</span><strong>${row.points||0}</strong></article>`}).join("")||"<p class=\"empty-state\">No archived results for this view.</p>"}</div>`}
function renderArchives(){const events=Object.entries(competition.archives||{}).flatMap(([seasonId,season])=>Object.entries(season||{}).map(([eventId,event])=>({seasonId,eventId,event}))).sort((a,b)=>(b.event.endedAt||0)-(a.event.endedAt||0));$("race-archive-list").innerHTML=events.map(({seasonId,eventId,event})=>`<article class="archive-card"><div><strong>${escapeHtml(event.name)}</strong><span>${escapeHtml(event.date)} • ${escapeHtml(event.seasonId)} • ${event.pointsEvent===false?"Non-points":"Points event"}</span></div><ol>${(event.classification||[]).map(row=>`<li><span>${escapeHtml(row.driverName||row.vehicleName||"Unassigned")}</span><b>${event.pointsEvent===false?"No points":`${row.points} pts`}</b></li>`).join("")}</ol><button class="action secondary-btn" data-delete-archive="${escapeHtml(seasonId)}/${escapeHtml(eventId)}" type="button">Delete result &amp; points</button></article>`).join("")||"<p class=\"empty-state\">No races have been archived.</p>";document.querySelectorAll("[data-delete-archive]").forEach(button=>button.onclick=()=>deleteArchivedResult(button.dataset.deleteArchive))}
async function deleteArchivedResult(key){if(!requireAuthenticatedWrite())return;const separator=key.indexOf("/"),seasonId=key.slice(0,separator),eventId=key.slice(separator+1),event=competition.archives?.[seasonId]?.[eventId];if(!event||!confirm(`Delete ${event.name} from the official archive?\n\nAny driver, team, vehicle, and passenger points from this event will be removed. This cannot be undone.`))return;const remaining={...(competition.archives?.[seasonId]||{})};delete remaining[eventId];await update(ref(db,"mfma"),{[`competition/archives/${seasonId}/${eventId}`]:null,[`competition/standings/${seasonId}`]:rebuildStandings(remaining)})}
async function archiveAndEndEvent(){
 ...
 if(!confirm(`Archive this ${pointsEvent?"points":"non-points"} event and end it?${passengerNotice}${nonPointsNotice}`))return;
 const seasonId=state.event.seasonId||currentSeasonId(),eventId=`${new Date().toISOString().slice(0,10)}-${slugify(state.event.name)}-${Date.now().toString(36)}`,archive=buildEventArchive(state,competition,{seasonId}),archives={...(competition.archives?.[seasonId]||{}),[eventId]:archive},standings=rebuildStandings(archives),endedState={systemState:"no-event",activeFlag:"clear",event:null,session:null,sprint:null,updatedAt:serverTimestamp()};
 try{await update(ref(db,"mfma"),{[`competition/seasons/${seasonId}/name`]:`${seasonId} Season`,[`competition/seasons/${seasonId}/points`]:(competition.seasons?.[seasonId]?.points||DEFAULT_POINTS).slice(0,2),[`competition/archives/${seasonId}/${eventId}`]:archive,[`competition/standings/${seasonId}`]:standings,state:endedState})}catch(error){console.error("Unable to archive event",error);if(confirm("The event archive is unavailable. End this event without saving its result?"))await set(stateRef,endedState)}
}
...
function renderReports(){
 const reports=state?.event?.vehicleReports||{},passengers=Object.values(state?.event?.passengerParticipants||{});
 $("occupant-panel").classList.toggle("hidden",!Object.keys(reports).length&&!passengers.length);$("occupant-list").innerHTML=Object.entries(reports).map(([vehicleId,r])=>`<article><strong>${escapeHtml(vehicleLabel(vehicleId))}</strong><span>Driver: ${escapeHtml(r.driverName||"—")}${r.mraNumber?` • ${escapeHtml(r.mraNumber)}`:""}</span><span>Passenger: ${escapeHtml(r.passengerName||"—")}</span><small>${relativeTime(r.submittedAt)}</small></article>`).join("")+passengers.map(passenger=>`<article><strong>Passenger Participation</strong><span>${escapeHtml(passenger.driverName)} • ${escapeHtml(passenger.mraNumber)}</span><span>1 Driver Championship point pending archive</span><small>Recorded by ${escapeHtml(competition.teams?.[passenger.recordedByTeamId]?.name||passenger.recordedByTeamId)}</small></article>`).join("");
