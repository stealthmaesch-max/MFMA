...
async function invalidateQualifyingLap(lapId){if(!requireAuthenticatedWrite()||!state?.event?.qualifying?.laps?.[lapId])return;const reason=normalizeName(prompt("Reason this qualifying lap is invalid:")||"");if(!reason)return;await update(stateRef,{[`event/qualifying/laps/${lapId}/status`]:"invalid",[`event/qualifying/laps/${lapId}/invalidReason`]:reason,[`event/qualifying/laps/${lapId}/invalidatedAt`]:serverTimestamp(),updatedAt:serverTimestamp()})}
async function endQualifying(){if(!requireAuthenticatedWrite()||state?.systemState!=="qualifying-live")return;const fastest=fastestQualifyingLap(state.event.qualifying?.laps);if(!confirm(fastest?`End qualifying? ${fastest.driverName} is fastest at ${fmtQualifying(fastest.timeMs)}.`:"End qualifying without a valid time?"))return;await update(stateRef,{systemState:"standby",activeFlag:"clear","event/qualifying/active":false,"event/qualifying/running":false,"event/qualifying/lastTickAt":null,"event/qualifying/completedAt":serverTimestamp(),"event/qualifying/winner":fastest?{driverId:fastest.driverId,driverName:fastest.driverName,timeMs:fastest.timeMs,lapId:fastest.id}:null,updatedAt:serverTimestamp()})}
...
async function adoptFairPolicy(season){if(!requireAuthenticatedWrite()||!confirm(`Adopt session points of 2–1 and at-large event points of 5–3 for ${season}? Passenger points will be capped at three, and the lowest at-large score will drop after six events. Existing adjustments remain unchanged.`))return;const archives=competition.archives?.[season]||{},adjustments=competition.pointsAdjustments?.[season]||{},standings=applyStandingsAdjustments(rebuildStandings(archives),adjustments);await update(ref(db,"mfma/competition"),{[`seasons/${season}/points`]:DEFAULT_POINTS,[`seasons/${season}/sessionPoints`]:DEFAULT_SESSION_POINTS,[`seasons/${season}/scoringPolicy`] :"session-and-at-large-v2",[`drivers/mra-16/competitionRole`]:"primary",[`drivers/mra-3/competitionRole`]:"honorary",[`standings/${season}`]:standings})}
...
async function discardInvalidEvent(){if(!requireAuthenticatedWrite()||!state?.event||state.systemState==="sprint-live")return;const issues=eventArchiveIssues();if(!issues.length)return alert("This event currently meets the basic archive requirements. Use End Event to review and archive it.");const reason=normalizeName(prompt(`Reason for deleting this invalid event:\n\n• ${issues.join("\n• ")}`)||"");if(!reason||!confirm(`Permanently end and delete the active event data?\n\n${state.event.name}\nReason: ${reason}\n\nNo result or points will be archived. This cannot be undone.`))return;const endedState={systemState:"no-event",activeFlag:"clear",event:null,session:null,sprint:null,updatedAt:serverTimestamp()},updates={state:endedState};if(state.event.championshipEventId)Object.assign(updates,{[`competition/events/${state.event.championshipEventId}/status`]:"cancelled",[`competition/events/${state.event.championshipEventId}/registrationStatus`]:"closed",[`competition/events/${state.event.championshipEventId}/cancelReason`]:reason,[`competition/events/${state.event.championshipEventId}/cancelledAt`]:serverTimestamp()});await update(ref(db,"mfma"),updates)}
async function archiveAndEndEvent(){
 ...
 if(!confirm(`Review official result:\n\n${resultPreview}\n\nArchive this ${pointsEvent?"points":"non-points"} event and end it?${passengerNotice}${sprintNotice}${nonPointsNotice}`))return;
...
function renderReports(){
 ...
 document.querySelectorAll("[data-approve-replacement]").forEach(button=>button.onclick=async()=>{const teamId=button.dataset.approveReplacement,reason=normalizeName(prompt(`Reason for ${competition.teams?.[teamId]?.name||teamId} vehicle replacement:`)||"");if(!reason||!confirm(`Approve this emergency replacement for championship scoring?\n\n${reason}`))return;await update(stateRef,{[`event/vehicleReplacements/${teamId}`]:{approved:true,reason,approvedBy:currentUser?.uid||null,approvedByName:displayUserName(currentUser),approvedAt:Date.now()},updatedAt:serverTimestamp()})});
...
function render(){
 ...
 const archiveIssues=!testMode?eventArchiveIssues():[],canDiscard=!testMode&&archiveIssues.length>0&&!sprintLive;$("discard-invalid-event").classList.toggle("hidden",!canDiscard);$("archive-issues").classList.toggle("hidden",!canDiscard);$("archive-issues").textContent=canDiscard?`Cannot legally archive: ${archiveIssues.join("; ")}.`:"";
