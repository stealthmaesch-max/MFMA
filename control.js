...
function render(){
 ...
 renderReports();
 const dedicatedTermination=safetyTerm||review||whiteTerm,hideEnd=sprintLive||course||live||staging||countdown||dedicatedTermination;$("standby-button").classList.toggle("hidden",sprintLive||course||live||prov||complete||staging||countdown||dedicatedTermination);$("end-event").classList.toggle("hidden",hideEnd);$("official-outcomes").classList.toggle("hidden",hideEnd||state.event.pointsEvent===false);const outcomeNames=state.event.teamNames||state.session?.teamNames||{},eventKey=`${state.event.name}|${state.event.sessionResults?.length||0}`;for(const side of ["a","b"]){$("outcome-label-"+side).firstChild.textContent=`${outcomeNames[side]||`Team ${side.toUpperCase()}`} `;const select=$("event-outcome-"+side);if(select.dataset.eventKey!==eventKey){const scores=state.event.scores||{},winner=Number(scores[side]||0)>Number(scores[side==="a"?"b":"a"]||0);select.value=winner?"winner":"classified";select.dataset.eventKey=eventKey}}
 if(standby||course){
