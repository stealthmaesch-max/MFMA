...
import { DEFAULT_POINTS, OFFICIAL_TEAM_IDS, applyStandingsAdjustments, approvedVehicles, buildEventArchive, rebuildStandings, currentSeasonId, driverCompetitionRole, isScoringDriverEligible, normalizeMraNumber, normalizeName, slugify, sortedStandings, teamBranding } from "./competition-model.js?v=83";
...
function renderStandings(){const season=$("standings-season").value||currentSeasonId(),type=$("standings-type").value,entries=season==="career"?Object.values(competition.archives||{}).flatMap(value=>Object.entries(value||{})):Object.entries(competition.archives?.[season]||{}),adjustments=season==="career"?Object.assign({},...Object.values(competition.pointsAdjustments||{})):competition.pointsAdjustments?.[season],source=applyStandingsAdjustments(rebuildStandings(Object.fromEntries(entries)),adjustments),rows=sortedStandings(source?.[type]),configured=competition.seasons?.[season]?.points||DEFAULT_POINTS,fair=configured[0]===5&&configured[1]===3,policy=season==="career"?"":`<section class="scoring-policy ${fair?"active":"needs-review"}"><div><strong>${fair?"Fair two-team policy active":"Scoring policy needs review"}</strong><span>${configured.slice(0,2).join("–")} points • Primary driver scores • One verified participation point</span></div>${fair?"":'<button id="adopt-fair-policy" class="action primary-btn" type="button">Adopt 5–3 Policy</button>'}</section>`;$("standings-list").innerHTML=policy+`<div class="standings-table"><div class="standings-head"><span>Pos</span><span>${type[0].toUpperCase()+type.slice(1)}</span><span>Wins</span><span>Points</span></div>${rows.map(row=>{const teamId=type==="teams"?row.id:type==="drivers"?competition.drivers?.[row.id]?.teamId:competition.vehicles?.[row.id]?.teamId,brand=teamBranding(teamId);return `<article style="--team-accent:${escapeHtml(brand.accent)}"><b>${row.championshipRank}${row.tied?"T":""}</b><span class="standing-name">${brand.emblem?`<img src="${escapeHtml(brand.emblem)}" alt="">`:""}<span>${escapeHtml(row.name)}</span></span><span>${row.wins||0}</span><strong>${row.points||0}</strong></article>`}).join("")||"<p class=\"empty-state\">No archived results for this view.</p>"}</div>`;if($("adopt-fair-policy"))$("adopt-fair-policy").onclick=()=>adoptFairPolicy(season);renderPointsAdjustments(season,type)}
async function adoptFairPolicy(season){if(!requireAuthenticatedWrite()||!confirm(`Adopt the 5–3 scoring policy for ${season}, mark MRA #16 as Monarch primary, mark Rockwell honorary, and rebuild stored standings? Existing manual adjustments remain auditable and unchanged.`))return;const archives=competition.archives?.[season]||{},adjustments=competition.pointsAdjustments?.[season]||{},standings=applyStandingsAdjustments(rebuildStandings(archives),adjustments);await update(ref(db,"mfma/competition"),{[`seasons/${season}/points`]:DEFAULT_POINTS,[`seasons/${season}/scoringPolicy`] :"two-team-fair-v1",[`drivers/mra-16/competitionRole`]:"primary",[`drivers/mra-3/competitionRole`]:"honorary",[`standings/${season}`]:standings})}
...
function renderPointsAdjustments(season,type){const form=$("points-adjustment-form"),disabled=season==="career",entrants=adjustmentEntrants(type),prior=$("points-adjustment-entrant").value;form.classList.toggle("hidden",disabled);$("points-adjustment-entrant").innerHTML=Object.entries(entrants).map(([id,item])=>`<option value="${escapeHtml(id)}">${escapeHtml(item.name||id)}</option>`).join("");if(entrants[prior])$("points-adjustment-entrant").value=prior;const ledger=Object.entries(competition.pointsAdjustments?.[season]||{}).sort((a,b)=>(b[1].createdAt||0)-(a[1].createdAt||0));$("points-adjustment-ledger").innerHTML=ledger.map(([id,item])=>`<article class="${item.status==="void"?"void-result":""}"><span><strong>${escapeHtml(item.entityName)}</strong><small>${escapeHtml(item.reason)}${item.status==="void"?" • VOIDED":""}</small></span><b>${item.points>0?"+":""}${item.points}</b>${item.status==="void"?"<em>Voided</em>":`<button class="mini-btn" data-void-adjustment="${escapeHtml(id)}">Void</button>`}</article>`).join("");document.querySelectorAll("[data-void-adjustment]").forEach(button=>button.onclick=()=>update(ref(db,`mfma/competition/pointsAdjustments/${season}/${button.dataset.voidAdjustment}`),{status:"void",voidedAt:serverTimestamp()}))}
async function submitPointsAdjustment(event){event.preventDefault();const season=$("standings-season").value,type=$("standings-type").value,entityId=$("points-adjustment-entrant").value,entity=adjustmentEntrants(type)[entityId],points=Number($("points-adjustment-value").value),reason=normalizeName($("points-adjustment-reason").value);if(season==="career"||!entity||!Number.isInteger(points)||points===0||!reason)return alert("Choose an entrant and enter a non-zero whole-point adjustment with a reason.");const id=`adjustment-${Date.now().toString(36)}`;await update(ref(db,`mfma/competition/pointsAdjustments/${season}/${id}`),{type,entityId,entityName:entity.name||entityId,points,reason,status:"active",createdAt:serverTimestamp()});event.target.reset();renderStandings()}
...
async function voidArchivedResult(key){if(!requireAuthenticatedWrite())return;const separator=key.indexOf("/"),seasonId=key.slice(0,separator),eventId=key.slice(separator+1),event=competition.archives?.[seasonId]?.[eventId],reason=event&&normalizeName(prompt(`Why is ${event.name} being voided?`)||"");if(!event||!reason)return;const archives={...(competition.archives?.[seasonId]||{}),[eventId]:{...event,status:"void",voidReason:reason,voidedAt:Date.now()}};await update(ref(db,"mfma"),{[`competition/archives/${seasonId}/${eventId}/status`]:"void",[`competition/archives/${seasonId}/${eventId}/voidReason`]:reason,[`competition/archives/${seasonId}/${eventId}/voidedAt`]:serverTimestamp(),[`competition/standings/${seasonId}`]:applyStandingsAdjustments(rebuildStandings(archives),competition.pointsAdjustments?.[seasonId])})}
async function archiveAndEndEvent(){
 if(!requireAuthenticatedWrite()||state?.systemState==="sprint-live"||!state?.event)return;
 const scores=Object.values(state.event.scores||{}).map(Number);if(scores.length!==2||scores[0]===scores[1])return alert("Resolve the finishing order before archiving. A points event cannot be archived with a tied or incomplete score.");
 ...
 const passengerNotice=pointsEvent&&passengerCount?`\n\nThis includes one passenger participation point for ${passengerCount} registered ${passengerCount===1?"passenger":"passengers"}.`:"";
 const nonPointsNotice=pointsEvent?"":"\n\nNo driver, team, vehicle, or passenger points will be awarded.";
 const seasonId=state.event.seasonId||currentSeasonId(),eventId=`${new Date().toISOString().slice(0,10)}-${slugify(state.event.name)}-${Date.now().toString(36)}`,archive=buildEventArchive(state,competition,{seasonId}),resultPreview=archive.classification.map(row=>`P${row.position} ${row.teamName}: ${row.driverName||"No driver"} / ${row.vehicleName}`).join("\n");
 ...
 if(!confirm(`Review official result:\n\n${resultPreview}\n\nArchive this ${pointsEvent?"points":"non-points"} event and end it?${passengerNotice}${nonPointsNotice}`))return;
 const archives={...(competition.archives?.[seasonId]||{}),[eventId]:archive},standings=applyStandingsAdjustments(rebuildStandings(archives),competition.pointsAdjustments?.[seasonId]),endedState={systemState:"no-event",activeFlag:"clear",event:null,session:null,sprint:null,updatedAt:serverTimestamp()},updates={[`competition/seasons/${seasonId}/name`]:`${seasonId} Season`,[`competition/seasons/${seasonId}/points`]:(competition.seasons?.[seasonId]?.points||DEFAULT_POINTS).slice(0,2),[`competition/archives/${seasonId}/${eventId}`]:archive,[`competition/standings/${seasonId}`]:standings,state:endedState};if(state.event.championshipEventId)Object.assign(updates,{[`competition/events/${state.event.championshipEventId}/status`]:"complete",[`competition/events/${state.event.championshipEventId}/registrationStatus`]:"closed",[`competition/events/${state.event.championshipEventId}/completedAt`]:serverTimestamp()});
 try{await update(ref(db,"mfma"),updates)}catch(error){console.error("Unable to archive event",error);if(confirm("The event archive is unavailable. End this event without saving its result?"))await set(stateRef,endedState)}
}
...
function renderReports(){
 ...
 $("occupant-panel").classList.toggle("hidden",!Object.keys(reports).length&&!passengers.length&&!drivers.length);$("occupant-list").innerHTML=drivers.map(driver=>{const role=driverCompetitionRole(driver.driverId,competition.drivers?.[driver.driverId]),eligible=isScoringDriverEligible(driver.driverId,competition.drivers?.[driver.driverId]),sessions=state.event.participationSessions?.[driver.driverId]||0;return `<article><strong>${escapeHtml(driver.driverName)} • ${escapeHtml(driver.teamName)}</strong><span>${escapeHtml(vehicleLabel(driver.vehicleId))} • MRA ${escapeHtml(driver.mraNumber)} • ${escapeHtml(role)}</span><small>${sessions} completed session${sessions===1?"":"s"}</small><button class="mini-btn" data-scoring-driver="${escapeHtml(driver.teamId)}/${escapeHtml(driver.driverId)}" ${eligible?"":"disabled"}>${eligible?(state.event.scoringDrivers?.[driver.teamId]===driver.driverId?"Scoring driver selected":"Use as scoring driver"):"Honorary — not scoring"}</button></article>`}).join("")+Object.entries(reports).map(([vehicleId,r])=>`<article><strong>${escapeHtml(vehicleLabel(vehicleId))}</strong><span>Current driver: ${escapeHtml(r.driverName||"—")}${r.mraNumber?` • ${escapeHtml(r.mraNumber)}`:""}</span><span>Current passenger: ${escapeHtml(r.passengerName||"—")}</span><small>${relativeTime(r.submittedAt)}</small></article>`).join("")+passengers.map(passenger=>{const sessions=state.event.participationSessions?.[passenger.driverId]||0;return `<article><strong>Passenger Participation</strong><span>${escapeHtml(passenger.driverName)} • ${escapeHtml(passenger.mraNumber)}</span><span>${state.event.pointsEvent===false?"Non-points event":sessions>0?"1 verified Driver Championship point":"Pending one completed session"}</span><small>${sessions} completed session${sessions===1?"":"s"} • Recorded by ${escapeHtml(competition.teams?.[passenger.recordedByTeamId]?.name||passenger.recordedByTeamId)}</small><button class="mini-btn" data-remove-passenger="${escapeHtml(passenger.driverId)}">Remove mistaken entry</button></article>`}).join("");
 ...
 document.querySelectorAll("[data-remove-passenger]").forEach(button=>button.onclick=()=>{if(confirm("Remove this passenger participation entry from the current event?"))update(stateRef,{[`event/passengerParticipants/${button.dataset.removePassenger}`]:null,updatedAt:serverTimestamp()})});
...
function render(){
 ...
 renderReports();
 const dedicatedTermination=safetyTerm||review||whiteTerm;$("standby-button").classList.toggle("hidden",sprintLive||course||live||prov||complete||staging||countdown||dedicatedTermination);$("end-event").classList.toggle("hidden",sprintLive||course||live||staging||countdown||dedicatedTermination);
 if(standby||course){
...
onValue(competitionRef,snapshot=>{competition=snapshot.val()||{};renderVehicleSetup();renderManagement();renderChampionshipEventOptions()});$("new-event-season").value=currentSeasonId();renderVehicleSetup();renderManagement();renderChampionshipEventOptions();setInterval(()=>{tick();sprintTick();if(state?.systemState==="next-session-countdown")countdownDots($("control-start-dots"),state.session.countdownEndsAt)},250);
