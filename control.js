...
async function discardInvalidEvent(){if(!requireAuthenticatedWrite()||!state?.event)return;const issues=eventArchiveIssues(),issueText=issues.length?`\n\nCurrent archive problems:\n• ${issues.join("\n• ")}`:"\n\nThe event may be legally archivable, but Race Management may still cancel it without a result.";const reason=normalizeName(prompt(`Reason for ending without a result:${issueText}`)||"");if(!reason||!confirm(`Permanently end and delete the active event data?\n\n${state.event.name}\nReason: ${reason}\n\nAny live Sprint or session will stop immediately. No result or points will be archived. This cannot be undone.`))return;const endedState={systemState:"no-event",activeFlag:"clear",event:null,session:null,sprint:null,updatedAt:serverTimestamp()},updates={state:endedState};if(state.event.championshipEventId)Object.assign(updates,{[`competition/events/${state.event.championshipEventId}/status`]:"cancelled",[`competition/events/${state.event.championshipEventId}/registrationStatus`]:"closed",[`competition/events/${state.event.championshipEventId}/cancelReason`]:reason,[`competition/events/${state.event.championshipEventId}/cancelledAt`]:serverTimestamp()});await update(ref(db,"mfma"),updates)}
async function archiveAndEndEvent(){
 if(!requireAuthenticatedWrite()||!state?.event)return;
 if(state.systemState==="sprint-live"){if(!confirm("End the active Sprint, verify eligible participation, and continue to the official event archive review?"))return;await closeSprintForEventEnd()}
