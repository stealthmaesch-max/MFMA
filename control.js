...
function renderReports(){
 ...
 $("hazard-panel").classList.toggle("hidden",!hazards.length);$("hazard-list").innerHTML=hazards.map(h=>`<article class="hazard-alert ${escapeHtml(h.status)}"><div class="hazard-alert-head"><b>HAZARD • ${escapeHtml((vehicles[h.vehicleId]?.name||h.vehicleId).toUpperCase())}</b><span>${h.status==="open"?"NEW":"ACKNOWLEDGED"}</span></div><strong>${escapeHtml(h.description)}</strong>${h.category?`<small>${escapeHtml(h.category)}</small>`:""}<div class="hazard-requests">${h.requestStopClock?"<b>STOP CLOCK</b>":""}${h.requestSafetyCar?"<b>SAFETY CAR</b>":""}</div><div class="button-row">${h.status==="open"?`<button class="mini-btn" data-hazard-ack="${h.id}">Acknowledge</button>`:""}${h.status!=="resolved"?`<button class="mini-btn" data-hazard-resolve="${h.id}">Resolve</button>`:""}${h.requestStopClock&&(state?.session?.running||state?.sprint?.running)?`<button class="mini-btn" data-hazard-stop="${h.id}">Stop Clock</button>`:""}${h.requestSafetyCar?`<button class="mini-btn" data-hazard-safety="${h.id}">Safety Car</button>`:""}</div></article>`).join("");
