...
import { DEFAULT_POINTS, OFFICIAL_TEAM_IDS, applyStandingsAdjustments, approvedVehicles, buildEventArchive, rebuildStandings, currentSeasonId, normalizeMraNumber, normalizeName, slugify, sortedStandings, teamBranding } from "./competition-model.js?v=81";
...
function renderStandings(){const season=$("standings-season").value||currentSeasonId(),type=$("standings-type").value,entries=season==="career"?Object.values(competition.archives||{}).flatMap(value=>Object.entries(value||{})):Object.entries(competition.archives?.[season]||{}),adjustments=season==="career"?Object.assign({},...Object.values(competition.pointsAdjustments||{})):competition.pointsAdjustments?.[season],source=applyStandingsAdjustments(rebuildStandings(Object.fromEntries(entries)),adjustments),rows=sortedStandings(source?.[type]);$("standings-list").innerHTML=`<div class="standings-table"><div class="standings-head"><span>Pos</span><span>${type[0].toUpperCase()+type.slice(1)}</span><span>Wins</span><span>Points</span></div>${rows.map((row,index)=>{const teamId=type==="teams"?row.id:type==="drivers"?competition.drivers?.[row.id]?.teamId:competition.vehicles?.[row.id]?.teamId,brand=teamBranding(teamId);return `<article style="--team-accent:${escapeHtml(brand.accent)}"><b>${index+1}</b><span class="standing-name">${brand.emblem?`<img src="${escapeHtml(brand.emblem)}" alt="">`:""}<span>${escapeHtml(row.name)}</span></span><span>${row.wins||0}</span><strong>${row.points||0}</strong></article>`}).join("")||"<p class=\"empty-state\">No archived results for this view.</p>"}</div>`;renderPointsAdjustments(season,type)}
...
async function archiveAndEndEvent(){
 ...
 for(const teamId of teamIds){const participants=Object.values(state.event.driverParticipants||{}).filter(driver=>driver.teamId===teamId);if(participants.length>1&&!state.event.scoringDrivers?.[teamId])return alert(`Choose the scoring driver for ${competition.teams?.[teamId]?.name||teamId} in Driver Reports before archiving.`)}
...
function renderReports(){
 ...
 $("occupant-panel").classList.toggle("hidden",!Object.keys(reports).length&&!passengers.length&&!drivers.length);$("occupant-list").innerHTML=drivers.map(driver=>`<article><strong>${escapeHtml(driver.driverName)} • ${escapeHtml(driver.teamName)}</strong><span>${escapeHtml(vehicleLabel(driver.vehicleId))} • MRA ${escapeHtml(driver.mraNumber)}</span><small>${relativeTime(driver.lastParticipatedAt)}</small><button class="mini-btn" data-scoring-driver="${escapeHtml(driver.teamId)}/${escapeHtml(driver.driverId)}">${state.event.scoringDrivers?.[driver.teamId]===driver.driverId?"Scoring driver selected":"Use as scoring driver"}</button></article>`).join("")+Object.entries(reports).map(([vehicleId,r])=>`<article><strong>${escapeHtml(vehicleLabel(vehicleId))}</strong><span>Current driver: ${escapeHtml(r.driverName||"—")}${r.mraNumber?` • ${escapeHtml(r.mraNumber)}`:""}</span><span>Current passenger: ${escapeHtml(r.passengerName||"—")}</span><small>${relativeTime(r.submittedAt)}</small></article>`).join("")+passengers.map(passenger=>`<article><strong>Passenger Participation</strong><span>${escapeHtml(passenger.driverName)} • ${escapeHtml(passenger.mraNumber)}</span><span>${state.event.pointsEvent===false?"Non-points event":"1 Driver Championship point pending archive"}</span><small>Recorded by ${escapeHtml(competition.teams?.[passenger.recordedByTeamId]?.name||passenger.recordedByTeamId)}</small><button class="mini-btn" data-remove-passenger="${escapeHtml(passenger.driverId)}">Remove mistaken entry</button></article>`).join("");
