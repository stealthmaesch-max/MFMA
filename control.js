...
import { enableSounds, getSoundStatus, onSoundStatus, playStateTransition, playSound } from "./sounds.js?v=74";
...
import { TRACK_SECTORS, TRACK_SIGNAL_TYPES, activeTrackSignals, trackOutlineMarkup } from "./track-signals.js?v=3";
...
const selectedTrackSectors=new Set(["sector-1"]);
...
function renderManagerDriverOptions(){
 ...
 $("manager-gator-reason-field").classList.toggle("hidden",$("manager-vehicle").value!=="gator");$("manager-gator-reason").value=stored?.gatorReason||"";
...
async function saveManagerDriverProfile(event){
 event.preventDefault();const teamId=linkedManagerTeamId(),driverId=$("manager-driver").value,passengerId=$("manager-passenger").value,vehicleId=$("manager-vehicle").value,gatorReason=normalizeName($("manager-gator-reason").value),driver=competition.drivers?.[driverId],passenger=competition.drivers?.[passengerId];if(managerBinding?.status!=="approved"||!driver||driver.teamId!==teamId||!["ranger","shelly","gator"].includes(vehicleId)){$("driver-manager-status").textContent="Link an approved Driver profile before saving.";return}if(vehicleId==="gator"&&!gatorReason){$("driver-manager-status").textContent="Explain why the emergency vehicle is required.";return}
 const profile={driverId,driverName:driver.name,mraNumber:driver.mraNumber,teamId,teamName:driver.teamName||competition.teams?.[teamId]?.name||"Team",vehicleId,gatorReason:vehicleId==="gator"?gatorReason:"",passengerId:passengerId||"",passengerName:passenger?.name||""};localStorage.setItem("mfma-driver-profile",JSON.stringify(profile));localStorage.setItem("mfma-driver-vehicle",vehicleId);localStorage.removeItem("mfma-driver-signed-out");
 if(state?.event){const emergencyReason=vehicleId==="gator"?gatorReason:null,updates={[`event/vehicleReports/${vehicleId}`]:{driverId,driverName:profile.driverName,mraNumber:profile.mraNumber,teamId,teamName:profile.teamName,passengerId:passengerId||null,passengerName:profile.passengerName,passengerMraNumber:passenger?.mraNumber||null,emergencyReason,submittedAt:serverTimestamp()},[`event/driverParticipants/${driverId}`]:{driverId,driverName:profile.driverName,mraNumber:profile.mraNumber,teamId,teamName:profile.teamName,vehicleId,emergencyReason,lastParticipatedAt:serverTimestamp()}};if(passenger)updates[`event/passengerParticipants/${passengerId}`]={driverId:passengerId,driverName:passenger.name,mraNumber:passenger.mraNumber,recordedByTeamId:teamId,participatedAt:serverTimestamp()};await update(stateRef,updates)}
...
function renderOfficialTeamSelectors(){const teams=OFFICIAL_TEAM_IDS.filter(id=>competition.teams?.[id]?.status==="approved"),options=teams.map(id=>`<option value="${escapeHtml(id)}">${escapeHtml(competition.teams[id].name)}</option>`).join("");for(const [elementId,fallbackIndex] of [["vv-team-id-1",0],["vv-team-id-2",1]]){const select=$(elementId),prior=select.value;select.innerHTML=options;select.value=teams.includes(prior)?prior:teams[fallbackIndex]||teams[0]||""}}
function renderVehicleSetup(){const catalog=vehicleCatalog(),ids=Object.keys(catalog),selected=new Set(availableVehicles());if(!selected.size){for(const id of ids)if(id!=="gator"&&selected.size<2)selected.add(id)}$("available-vehicles").innerHTML=ids.map(id=>`<label class="chip ${id==="gator"?"emergency-vehicle":""}"><input data-available-vehicle type="checkbox" value="${escapeHtml(id)}" ${selected.has(id)?"checked":""}> <span>${escapeHtml(catalog[id].name||id)}<small>${id==="gator"?"Emergency option":escapeHtml(vehicleTeamName(id))}</small></span></label>`).join("");document.querySelectorAll("[data-available-vehicle]").forEach(input=>input.onchange=renderVVSelectors);renderOfficialTeamSelectors();renderVVSelectors()}
...
function validate(){
 ...
 if($("vv-team-1").value===$("vv-team-2").value)errors.push("Choose two different vehicles.");
...
async function createEvent(){
 ...
 const eventFormat=championshipEvent?.eventFormat||$("new-event-format").value||"regular",pointsEvent=championshipEvent?championshipEvent.pointsEvent!==false:$("new-event-points").value==="points";
 await set(stateRef,{systemState:"standby",activeFlag:"clear",event:{name:championshipEvent?.name||$("new-event-name").value||"MFMA Event",eventFormat,seasonId,championshipEventId,pointsEvent,scores:{},sessionNumber:1,circuit:{format:null,roleIndex:0,roles:{}},pendingAdjustment:null,courseLap:{required:eventFormat==="regular",status:eventFormat==="regular"?"pending":"not-required"}},session:null,sprint:null,updatedAt:serverTimestamp()});
...
async function startQualifying(){if(!requireAuthenticatedWrite()||state?.systemState!=="standby")return;const driverId=$("qualifying-driver").value,driver=competition.drivers?.[driverId],trackLength=$("qualifying-track").value,lapLimit=Math.min(10,Math.max(2,Number($("qualifying-lap-limit").value)||5));if(!driver||!QUALIFYING_TRACKS[trackLength])return alert("Choose an approved driver and track length.");await update(stateRef,{systemState:"qualifying-live",activeFlag:"green","event/qualifying":{active:true,phase:"warmup",warmupCompleted:false,lapLimit,trackLength,vehicleId:"shelly",currentDriverId:driverId,currentDriverName:driver.name,currentMraNumber:driver.mraNumber||null,running:false,elapsedMs:0,lastTickAt:null,laps:{},startedAt:Date.now()},updatedAt:serverTimestamp()})}
async function selectQualifyingDriver(){if(!requireAuthenticatedWrite()||state?.systemState!=="qualifying-live"||state.event.qualifying?.running)return;const driverId=$("qualifying-live-driver").value,driver=competition.drivers?.[driverId];if(!driver)return;await update(stateRef,{"event/qualifying/currentDriverId":driverId,"event/qualifying/currentDriverName":driver.name,"event/qualifying/currentMraNumber":driver.mraNumber||null,"event/qualifying/elapsedMs":0,"event/qualifying/lastTickAt":null,activeFlag:state.event.qualifying?.warmupCompleted?"clear":"green",updatedAt:serverTimestamp()})}
async function completeQualifyingWarmup(){if(!requireAuthenticatedWrite()||state?.systemState!=="qualifying-live"||state.event.qualifying?.running||state.event.qualifying?.warmupCompleted)return;await update(stateRef,{"event/qualifying/warmupCompleted":true,"event/qualifying/phase":"timed","event/qualifying/warmupCompletedAt":serverTimestamp(),activeFlag:"clear",updatedAt:serverTimestamp()})}
function qualifyingDriverLapCount(qualifying,driverId){return Object.values(qualifying?.laps||{}).filter(lap=>lap?.driverId===driverId).length}
async function startQualifyingRun(){if(!requireAuthenticatedWrite()||state?.systemState!=="qualifying-live"||state.event.qualifying?.running)return;const qualifying=state.event.qualifying;if(!qualifying?.warmupCompleted)return alert("Complete the warm-up lap before starting timed qualifying.");if(qualifyingDriverLapCount(qualifying,qualifying.currentDriverId)>=(qualifying.lapLimit||5))return alert(`${qualifying.currentDriverName} has completed all ${qualifying.lapLimit||5} timed laps.`);await update(stateRef,{"event/qualifying/running":true,"event/qualifying/phase":"timed","event/qualifying/lastTickAt":Date.now(),activeFlag:"green",updatedAt:serverTimestamp()})}
...
async function finishQualifyingRun(){if(!requireAuthenticatedWrite()||state?.systemState!=="qualifying-live"||!state.event.qualifying?.running)return;const qualifying=state.event.qualifying,timeMs=qualifyingTime(qualifying),lapId=`lap-${Date.now().toString(36)}`,completed=qualifyingDriverLapCount(qualifying,qualifying.currentDriverId)+1,halfway=completed===Math.ceil((qualifying.lapLimit||5)/2);if(timeMs<1000)return alert("A qualifying lap must run for at least one second.");await update(stateRef,{[`event/qualifying/laps/${lapId}`]:{driverId:qualifying.currentDriverId,driverName:qualifying.currentDriverName,mraNumber:qualifying.currentMraNumber||null,teamId:competition.drivers?.[qualifying.currentDriverId]?.teamId||null,vehicleId:"shelly",trackLength:qualifying.trackLength,lapNumber:completed,timeMs,status:"valid",recordedAt:Date.now()},"event/qualifying/running":false,"event/qualifying/elapsedMs":0,"event/qualifying/lastTickAt":null,activeFlag:halfway?"qualifying-halfway":"checkered",updatedAt:serverTimestamp()})}
...
async function applyTrackSignal(type,sectors=[...selectedTrackSectors]){if(!requireAuthenticatedWrite()||!trackSignalAllowed()||!TRACK_SIGNAL_TYPES[type]||!sectors.length)return;const issuedAt=Date.now(),updates={updatedAt:serverTimestamp()};if(state.activeFlag==="grip-deterioration"){const sessionFlag=new Set(["green","yellow","red","safety-car"]).has(state.session?.flag)?state.session.flag:null;updates.activeFlag=sessionFlag||(state.systemState==="sprint-live"?"green":"clear");updates.gripCondition=null}for(const sector of sectors)if(TRACK_SECTORS.some(item=>item.id===sector))updates[`trackSignals/${sector}`]={type,issuedAt};await update(stateRef,updates)}
async function clearTrackSignals(sectors=[...selectedTrackSectors]){if(!requireAuthenticatedWrite()||!trackSignalAllowed()||!sectors.length)return;const updates={updatedAt:serverTimestamp()};for(const sector of sectors)updates[`trackSignals/${sector}`]=null;await update(stateRef,updates)}
async function issueGrip(condition="rain"){return applyTrackSignal(condition==="debris"?"debris":"rain",TRACK_SECTORS.map(sector=>sector.id))}
function toggleTrackSector(id){if(!TRACK_SECTORS.some(sector=>sector.id===id))return;if(selectedTrackSectors.has(id)&&selectedTrackSectors.size>1)selectedTrackSectors.delete(id);else selectedTrackSectors.add(id);renderTrackControl()}
function renderTrackControl(){const panel=$("track-control"),signals=activeTrackSignals(state?.trackSignals),signalClasses=Object.values(TRACK_SIGNAL_TYPES).map(signal=>signal.className);if(!panel)return;panel.classList.toggle("hidden",!trackSignalAllowed());panel.querySelectorAll("[data-track-sector]").forEach(item=>{const sector=item.dataset.trackSector,signal=signals.find(value=>value.id===sector);item.setAttribute("class",signal?.className||"");item.classList.toggle("selected",selectedTrackSectors.has(sector));item.setAttribute("aria-pressed",String(selectedTrackSectors.has(sector)))});panel.querySelectorAll("[data-sector-select]").forEach(button=>{const signal=signals.find(value=>value.id===button.dataset.sectorSelect);button.classList.remove(...signalClasses);if(signal)button.classList.add(signal.className);button.setAttribute("aria-pressed",String(selectedTrackSectors.has(button.dataset.sectorSelect)))});$("track-active-summary").textContent=signals.length?signals.map(signal=>`${signal.label} • ${signal.shortLabel}`).join(" · "):"No local track signals"}
function installGripControls(){const quick=$("quick-flag-panel");if(quick&&!$("track-control")){const panel=document.createElement("details");panel.id="track-control";panel.className="track-control";panel.innerHTML=`<summary><span><b>Track Sectors</b><small id="track-active-summary">No local track signals</small></span><strong>Open Map</strong></summary><div class="track-control-body">${trackOutlineMarkup("track-control-map")}<div class="track-sector-buttons">${TRACK_SECTORS.map(sector=>`<button type="button" data-sector-select="${sector.id}" aria-pressed="${selectedTrackSectors.has(sector.id)}"><b>${sector.label}</b><span>${sector.name}</span></button>`).join("")}</div><div class="track-signal-actions">${Object.entries(TRACK_SIGNAL_TYPES).map(([type,signal])=>`<button type="button" data-track-signal="${type}" class="${signal.className}">${signal.label}</button>`).join("")}<button type="button" data-track-clear>Clear Selected</button><button type="button" data-track-clear-all>Clear All</button></div></div>`;quick.append(panel);panel.querySelectorAll("[data-sector-select]").forEach(button=>button.onclick=()=>toggleTrackSector(button.dataset.sectorSelect));panel.querySelectorAll("[data-track-sector]").forEach(path=>{path.setAttribute("role","button");path.setAttribute("tabindex","0");path.onclick=()=>toggleTrackSector(path.dataset.trackSector);path.onkeydown=event=>{if(event.key==="Enter"||event.key===" "){event.preventDefault();toggleTrackSector(path.dataset.trackSector)}}});panel.querySelectorAll("[data-track-signal]").forEach(button=>button.onclick=()=>applyTrackSignal(button.dataset.trackSignal));panel.querySelector("[data-track-clear]").onclick=()=>clearTrackSignals();panel.querySelector("[data-track-clear-all]").onclick=()=>clearTrackSignals(TRACK_SECTORS.map(sector=>sector.id))}for(const [selector,compact] of [["#secondary-signals",true],["#standby-panel .flag-grid",false],["#sprint-panel .flag-grid",false],["#live-flag-panel",false],["#test-mode-panel .flag-grid",false]]){const host=document.querySelector(selector);if(!host||host.querySelector("[data-grip-condition]"))continue;for(const [condition,label] of [["rain","Rain / Water"],["debris","Loose Debris"]]){const button=document.createElement("button");button.type="button";button.className="flag grip";button.dataset.gripCondition=condition;button.innerHTML=`<b>ALL SECTORS</b><strong>Grip • ${label}</strong><small>${compact?"":"Keeps the primary flag active"}</small>`;button.onclick=()=>issueGrip(condition);host.append(button)}}}
...
function eventArchiveIssues(){
 ...
 const format=state.event.eventFormat||"regular";
 if(format==="qualifying")return state.event.qualifying?.winner?[]:["no valid qualifying result has been recorded"];
 if(format==="sprint")return state.event.pointsEvent!==false&&!Object.keys(state.event.sprintParticipants||{}).length?["no verified 60-second Sprint participant has been recorded"]:[];
...
async function archiveStandaloneFormat(){const seasonId=state.event.seasonId||currentSeasonId(),eventId=`${new Date().toISOString().slice(0,10)}-${slugify(state.event.name)}-${Date.now().toString(36)}`,archive=buildEventArchive(state,competition,{seasonId,finalOutcomes:{}}),format=state.event.eventFormat||"regular",pointsEvent=state.event.pointsEvent!==false;archive.eventFormat=format;archive.classification=[];const participationCount=archive.participationAwards?.length||0,qualifyingWinner=state.event.qualifying?.winner;if(format==="qualifying"&&!qualifyingWinner)return alert("Record at least one valid qualifying lap before archiving Qualifying.");if(format==="sprint"&&pointsEvent&&!participationCount)return alert("A points Sprint needs at least one verified 60-second participant before it can be archived.");const summary=format==="qualifying"?`${qualifyingWinner.driverName} won Qualifying in ${fmtQualifying(qualifyingWinner.timeMs)}.${pointsEvent?` ${participationCount} driver${participationCount===1?"":"s"} will receive qualifying points.`:" This is a non-points qualifying."}`:`${participationCount} verified Sprint participant${participationCount===1?"":"s"}.`;if(!await requestConfirmation("Archive and End Event",`${summary}\n\nArchive this ${format} and return to No Active Event?`,"Archive Event"))return;const archives={...(competition.archives?.[seasonId]||{}),[eventId]:archive},standings=applyStandingsAdjustments(rebuildStandings(archives),competition.pointsAdjustments?.[seasonId]),updates={[`competition/archives/${seasonId}/${eventId}`]:archive,[`competition/standings/${seasonId}`]:standings,state:{systemState:"no-event",activeFlag:"clear",event:null,session:null,sprint:null,updatedAt:serverTimestamp()}};if(state.event.championshipEventId)Object.assign(updates,{[`competition/events/${state.event.championshipEventId}/status`]:"complete",[`competition/events/${state.event.championshipEventId}/registrationStatus`]:"closed",[`competition/events/${state.event.championshipEventId}/completedAt`]:serverTimestamp()});await update(ref(db,"mfma"),updates)}
async function archiveAndEndEvent(){
 ...
 if(new Set(["sprint","qualifying"]).has(state.event.eventFormat))return archiveStandaloneFormat();
...
function render(){
 const mode=getRenderMode(state),has=mode!=="no-event",management=$("mra-management"),managementKey=state?.event?`${state.event.name||"event"}:${state.event.createdAt||state.event.date||"active"}`:null;document.body.dataset.mode=mode;document.body.classList.toggle("flag-controls-active",["standby","session-live","sprint-live"].includes(mode));E.noEvent.classList.toggle("hidden",has);E.eventArea.classList.toggle("hidden",!has);if(!has){management.open=true;managementEventKey=null}else if(managementEventKey!==managementKey){management.open=false;managementEventKey=managementKey}const safetyActive=safetyManagementActive(),currentSafetyMessage=safetyActive&&state.safetyMessage?.flag===state.activeFlag?state.safetyMessage.text||"":"";$("safety-message-open").classList.toggle("hidden",!safetyActive);$("safety-message-open").textContent=currentSafetyMessage?"Safety Info • Live":"Safety Info";if(!has)return;
 const eventFormat=state.event.eventFormat||"regular",formatLabel={regular:"Regular Event",sprint:"Sprint",qualifying:"Qualifying"}[eventFormat]||"Regular Event";E.eventName.textContent=state.event.name;E.eventMeta.textContent=state.event.circuit?.format?`${formatLabel} • ${state.event.circuit.format.replace("-","–")}`:formatLabel;
 ...
 E.setup.classList.toggle("hidden",!standby||eventFormat!=="regular");$("event-score-panel").classList.toggle("hidden",eventFormat!=="regular"||!(standby||(live&&!awaiting)));
 document.querySelector(".sprint-roster-card")?.classList.toggle("hidden",eventFormat!=="sprint");$("start-sprint").classList.toggle("hidden",eventFormat!=="sprint");document.querySelector(".qualifying-launch")?.classList.toggle("hidden",eventFormat==="sprint");
 ...
 const dedicatedTermination=safetyTerm||review||whiteTerm,hideOutcomes=sprintLive||qualifyingLive||course||live||staging||countdown||dedicatedTermination;$("standby-button").classList.toggle("hidden",sprintLive||qualifyingLive||course||live||prov||complete||staging||countdown||dedicatedTermination);$("end-event").classList.toggle("hidden",testMode);$("official-outcomes").classList.toggle("hidden",hideOutcomes||state.event.pointsEvent===false||eventFormat!=="regular");const outcomeNames=state.event.teamNames||state.session?.teamNames||{},qualifyingTeamId=state.event.qualifying?.winner?.teamId||null,eventKey=`${state.event.name}|${state.event.sessionResults?.length||0}|${qualifyingTeamId||""}`;for(const side of ["a","b"]){$("outcome-label-"+side).firstChild.textContent=`${outcomeNames[side]||`Team ${side.toUpperCase()}`} `;const select=$("event-outcome-"+side);if(select.dataset.eventKey!==eventKey){const scores=state.event.scores||{},opponent=side==="a"?"b":"a",tied=Number(scores[side]||0)===Number(scores[opponent]||0),winner=tied&&qualifyingTeamId?state.event.teamIds?.[side]===qualifyingTeamId:Number(scores[side]||0)>Number(scores[opponent]||0);select.value=winner?"winner":"classified";select.dataset.eventKey=eventKey}}
 if(standby||course){
  const lap=state.event?.courseLap||{};
  if(standby)renderSprintRoster();
  const qualifyingWinner=state.event?.qualifying?.winner,scores=state.event?.scores||{},scoreKeys=Object.keys(scores),eventTied=eventFormat==="regular"&&scoreKeys.length===2&&Number(scores[scoreKeys[0]]||0)===Number(scores[scoreKeys[1]]||0),qualifyingNotice=qualifyingWinner?`${eventTied?"Tiebreaker active":"Qualifying winner"}: ${qualifyingWinner.driverName} • ${fmtQualifying(qualifyingWinner.timeMs)} • ${QUALIFYING_TRACKS[state.event.qualifying.trackLength]} track • Shelly`:eventTied?"Regular Event tied • Complete Qualifying to determine the official winner":"";$("qualifying-summary").classList.toggle("hidden",!standby||!qualifyingNotice);$("qualifying-summary").textContent=qualifyingNotice;
 ...
 if(qualifyingLive){const qualifying=state.event.qualifying||{},laps=rankedQualifyingLaps(qualifying.laps),fastest=laps[0],completed=qualifyingDriverLapCount(qualifying,qualifying.currentDriverId),limit=qualifying.lapLimit||5,warmup=!qualifying.warmupCompleted;$("qualifying-track-label").textContent=QUALIFYING_TRACKS[qualifying.trackLength]||qualifying.trackLength||"—";$("qualifying-live-track").value=`${QUALIFYING_TRACKS[qualifying.trackLength]||"—"} • Shelly`;$("qualifying-driver-name").textContent=qualifying.currentDriverName||"Choose driver";$("qualifying-timer-status").textContent=warmup?"WARM-UP":qualifying.running?"RUNNING":completed>=limit?"LAPS COMPLETE":"READY";$("qualifying-progress").textContent=warmup?"Complete one untimed warm-up lap before timed running.":`${qualifying.currentDriverName||"Driver"} • ${completed} of ${limit} timed laps • Halfway signal after lap ${Math.ceil(limit/2)}`;$("qualifying-complete-warmup").classList.toggle("hidden",!warmup);$("qualifying-start-run").classList.toggle("hidden",warmup);$("qualifying-start-run").disabled=Boolean(qualifying.running)||completed>=limit;$("qualifying-finish-run").disabled=!qualifying.running;$("qualifying-reset-run").disabled=warmup;$("qualifying-live-driver").disabled=Boolean(qualifying.running);if(competition.drivers?.[qualifying.currentDriverId])$("qualifying-live-driver").value=qualifying.currentDriverId;$("qualifying-fastest").textContent=fastest?`${fastest.driverName} • ${fmtQualifying(fastest.timeMs)}`:"No valid time";$("qualifying-results").innerHTML=laps.map((lap,index)=>`<article><b>${index+1}</b><span><strong>${escapeHtml(lap.driverName)}</strong><small>Lap ${lap.lapNumber||"—"} of ${limit} • MRA ${escapeHtml(lap.mraNumber||"—")} • Shelly</small></span><strong>${fmtQualifying(lap.timeMs)}</strong><button class="mini-btn" data-invalid-lap="${escapeHtml(lap.id)}">Invalidate</button></article>`).join("")||'<p class="empty-state">No valid qualifying laps recorded.</p>';document.querySelectorAll("[data-invalid-lap]").forEach(button=>button.onclick=()=>invalidateQualifyingLap(button.dataset.invalidLap))}
 if(live){const s=state.session,spotsEnabled=s.phase==="finding";E.phase.textContent=s.phase==="hiding"?"HIDING":awaiting?"HIDING COMPLETE":"FINDING";E.timer.textContent=fmt(s.remainingMs);E.sessionLabel.textContent=`Session ${s.number}`;E.roles.textContent=`${s.teamNames[s.pursuitTeam]} pursuing • ${s.teamNames[s.evadingTeam]} evading`;E.active.textContent=signals[state.activeFlag]?.label||state.activeFlag;E.badge.textContent=awaiting?"AWAITING MRA STEWARD":s.running?"SESSION LIVE":"SESSION PAUSED";E.findingStart.classList.toggle("hidden",!awaiting);$("live-signal-panel").classList.toggle("hidden",awaiting);$("live-flag-panel").classList.toggle("hidden",awaiting);$("live-spots-panel").classList.toggle("hidden",awaiting);E.spots.innerHTML=(s.pursuitVehicleIds||[]).map(v=>`<button class="spot ${s.spotStatus?.[v]?"confirmed":""}" data-spot="${v}" ${s.spotStatus?.[v]||!spotsEnabled?"disabled":""}><span>${escapeHtml(vehicleLabel(v))}</span><strong>${s.spotStatus?.[v]?"SPOT CONFIRMED":spotsEnabled?"CONFIRM VALID RADIO SPOT":"FINDING NOT STARTED"}</strong></button>`).join("");E.spots.querySelectorAll("[data-spot]").forEach(b=>b.onclick=()=>confirmSpot(b.dataset.spot))}
...
$("swap-teams").onclick=()=>{roleIndex=roleIndex%2===0?1:0;updateRoleSummary()};
function openEventDialog(format){const copy={regular:["Start Regular Event","Official sessions determine the result. Qualifying is used only to break a tied Regular Event.","MFMA Event","Open Regular Event"],sprint:["Start Sprint","A participation competition with no finishing order. Eligible drivers can earn one capped Sprint participation point.","MFMA Sprint","Open Sprint"],qualifying:["Start Qualifying","A standalone Shelly time trial with a warm-up and 2–10 timed laps. Each timed driver earns 1 participation point and fastest lap earns 1 additional point.","MFMA Qualifying","Open Qualifying"]}[format]||[];$("new-event-format").value=format;$("new-event-title").textContent=copy[0];$("new-event-description").textContent=copy[1];$("new-event-name").value=copy[2];$("new-event-submit").textContent=copy[3];$("new-event-points").value="points";$("new-event-points").disabled=false;$("event-dialog").showModal();renderChampionshipEventOptions()}
document.querySelectorAll("[data-create-event]").forEach(button=>button.onclick=()=>openEventDialog(button.dataset.createEvent));$("close-dialog").onclick=()=>$("event-dialog").close();$("event-form").onsubmit=e=>{e.preventDefault();createEvent()};$("new-event-championship").onchange=updateChampionshipEventPreview;
...
$("start-qualifying").onclick=startQualifying;$("qualifying-live-driver").onchange=selectQualifyingDriver;$("qualifying-complete-warmup").onclick=completeQualifyingWarmup;$("qualifying-start-run").onclick=startQualifyingRun;$("qualifying-finish-run").onclick=finishQualifyingRun;$("qualifying-reset-run").onclick=resetQualifyingRun;$("end-qualifying").onclick=endQualifying;
...
$("driver-manager-open").onclick=()=>{renderManagerDriverOptions();$("driver-manager-dialog").showModal()};$("driver-manager-close").onclick=()=>$("driver-manager-dialog").close();$("driver-manager-form").onsubmit=saveManagerDriverProfile;$("manager-driver").onchange=()=>renderManagerPassengerOptions($("manager-driver").value,$("manager-passenger").value);$("manager-vehicle").onchange=()=>$("manager-gator-reason-field").classList.toggle("hidden",$("manager-vehicle").value!=="gator");

onValue(stateRef,s=>{const previous=state;state=s.val()||{systemState:"no-event"};playStateTransition(previous,state);scheduleWarningReturn();scheduleNextStart();const newHazards=newOpenHazardIds(previous,state);if(newHazards.length){try{playSound("hazard")}catch{}if(new Set(["clear","green","move-over"]).has(state.activeFlag||"clear"))issueFlag("yellow").catch(error=>console.error("Unable to call immediate yellow",error))}setConn("connected","Connected");roleIndex=state.event?.circuit?.roleIndex||roleIndex;if(!state.event){$("toolbar-event-name").textContent="MRA Steward";$("toolbar-state").textContent="NO EVENT";$("toolbar-flag").textContent="CLEAR";$("toolbar-timer").textContent="--:--"}render()},e=>{setConn("error","Connection error");console.error(e)});
