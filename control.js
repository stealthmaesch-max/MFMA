...
import { enableSounds, getSoundStatus, onSoundStatus, playStateTransition, playSound } from "./sounds.js?v=73";
...
import { TRACK_SECTORS, TRACK_SIGNAL_TYPES, activeTrackSignals, trackOutlineMarkup } from "./track-signals.js?v=1";
...
function render(){
 ...
 const flagsAllowed=standby||sprintLive||(live&&!awaiting)||safetyTerm;$("quick-flag-panel").classList.toggle("hidden",!flagsAllowed);$("quick-flag-panel").classList.toggle("track-only",safetyTerm);$("quick-flag-status").textContent=(state.activeFlag||"clear").replaceAll("-"," ").toUpperCase();
 const permittedFlags=safetyTerm?new Set():standby?new Set(["yellow","move-over","red","safety-car","checkered","clear"]):sprintLive?new Set(["green","yellow","move-over","red","safety-car","checkered","clear"]):new Set(["green","yellow","move-over","red","safety-car","checkered"]),secondaryFlags=new Set(["move-over","checkered","clear"]),secondaryActive=secondaryFlags.has(state.activeFlag);
 document.querySelectorAll("[data-quick-flag]").forEach(button=>{const permitted=permittedFlags.has(button.dataset.quickFlag);button.classList.toggle("hidden",!permitted);button.disabled=!permitted;button.classList.toggle("active",button.dataset.quickFlag===state.activeFlag)});$("more-signals").classList.toggle("hidden",safetyTerm);$("secondary-signals").classList.toggle("hidden",safetyTerm||!signalsExpanded&&!secondaryActive);$("more-signals").querySelector("strong").textContent=signalsExpanded||secondaryActive?"Less":"More";renderTrackControl();
